...
#define INITIAL_VALUE_HRM                       0                   // 心率测量初始值

/*-------心率测量flag位段定义，根据SIG发布的《HRS_SPEC_VV10r00》定义----------*/
#define HRM_FLAG_MASK_HR_VALUE_16BIT            (0x01 << 0)         // 心率值数据格式位 =0:UINT8; =1:UINT16
#define HRM_FLAG_MASK_SENSOR_CONTACT_DETECTED   (0x01 << 1)         // 传感器接触检测位，如果支持接触检测，当设备检测到没有接触到皮肤或者接触不良，位1应设置为0，否则，设置为1
#define HRM_FLAG_MASK_SENSOR_CONTACT_SUPPORTED  (0x01 << 2)         // 传感器接触支持位 =0:不支持传感器接触检测，=1: 支持
#define HRM_FLAG_MASK_EXPENDED_ENERGY_INCLUDED  (0x01 << 3)         // 能量消耗状态位，指示能量消耗字段是否存在于心率测量特征中。如果存在，该位应设置为1，否则设置为0
#define HRM_FLAG_MASK_RR_INTERVAL_INCLUDED      (0x01 << 4)         // RR-Interval位，指示RR-Interval值是否存在于心率测量特征中。如果一个或者多个RR-Interval值存在，该位应设置为1，否则设置为0
 ...
 * @param[in]   p_ble_evt   Event received from the BLE stack.
 */
static viod on_write(ble_hrs_t * p_hrs, ble_evt_t const * p_ble_evt)
{
...
uint32_t ble_hrs_init(ble_hrs_t * p_hrs, const ble_hrs_init_t * p_hrs_init)
{
    ...
    p_hrs->evt_handler                      = p_hrs_init->evt_handler;                  // 拷贝心率服务初始化结构体中的心率服务事件句柄
    p_hrs->is_sensor_contact_supported      = p_hrs_init->is_sensor_contact_supported;  // 拷贝心率服务初始化结构体中的心率服务事件句柄
    p_hrs->conn_handle                      = BLE_CONN_HANDLE_INVALID;                  // 连接句柄初始值为：无效的连接句柄
    p_hrs->is_sensor_contact_detected       = false;                                    // 传感器接触设置为false，即未接触
    p_hrs->rr_interval_count                = 0;                                        // rr_interval数量初始化值设置为0
    p_hrs->max_hrm_len                      = MAX_HRM_LEN;                              // 设置当前一次可发送的最大心率测量数据长度
    ...
    add_char_params.uuid                = BLE_UUID_HEART_RATE_MEASUREMENT_CHAR;         // 心率测量特征的UUID
    add_char_params.max_len             = MAX_HRM_LEN;                                  // 设置心率测量特征值的最大长度
    add_char_params.init_len            = 0;                                            // 设置心率测量特征值的初始长度
    add_char_params.p_init_value        = initial_hrm;
    add_char_params.is_var_len          = true;                                         // 设置心率测量的特征长度为可变长度
    add_char_params.char_props.notify   = 1;                                            // 设置传感器位置特征的性质：支持通知
    ...
    if (p_hrs_init->p_body_sensor_location != NULL) {
        ...
        add_char_params.uuid            = BLE_UUID_BODY_SENSOR_LOCATION_CHAR;                           // 设置传感器位置特征的UUID
        add_char_params.max_len         = sizeof(uint8_t);                                              // 设置传感器位置特征值最大长度
        add_char_params.init_len        = sizeof(uint8_t);                                              // 设置传感器位置特征值初始长度
        add_char_params.p_init_value    = p_hrs_init->p_body_sensor_location;                           // 设置传感器位置特征的初始值
        add_char_params.char_props.read = 1;                                                            // 设置传感器位置特征的性质：支持读取
        add_char_params.read_access     = p_hrs_init->bsl_rd_sec;                                       // 设置传感器位置特征值的安全需求
...
uint32_t ble_hrs_heart_rate_measurement_send(ble_hrs_t * p_hrs, uint16_t heart_rate)
{
    ...
    if (p_hrs->conn_handle != BLE_CONN_HANDLE_INVALID) {
        ...
        hvx_params.handle = p_hrs->hrm_handles.value_handle;            // 心率测量特征值句柄
        hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;                  // 类型为通知
        hvx_params.offset = 0;
        hvx_params.p_len  = &hvx_len;                                   // 发送的数据长度
        hvx_params.p_data = hrm_data;                                   // 发送的数据
